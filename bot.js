...
bot.onText(/^\/start(?:\s+(.+))?$/, async (msg, match) => {
    ...
    if (clientId) {
        ...
        await bot.sendMessage(chatId,
            `Вітаємо! 👋\n\n` +
            `Ви зареєструвались на курс по створенню контенту за допомогою нейромереж.\n\n` +
...
bot.on('callback_query', async (query) => {
    ...
    if (action === 'approve') {
        await bot.sendMessage(chatId, 
            ...
            'Старт курсу <b>02.02.26</b>. Напередодні старту курсу ми додамо вас в закриту групу ТГ, де будуть всі матеріали та подальша інформація. ☺️\n\n' +
            'А поки ви очікуєте старт курсу, можете вже зараз приєднатися у мій закритий ТГ-канал, де вже є багато безкоштовної інформації та однодумців. ♥️',
            { 
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [[
